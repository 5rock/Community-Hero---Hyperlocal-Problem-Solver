...
def get_badge(reports: int):
    if reports >= 100: return "Community Champion"
    if reports >= 50: return "Gold Hero"
    if reports >= 20: return "Silver Hero"
    if reports >= 5: return "Bronze Hero"
    return "Newcomer"

@router.get("/")
def get_admin_stats(current_user: models.User = Depends(RoleChecker("Admin")), db: Session = Depends(get_db)):
    total_issues = db.query(models.Issue).count()
    active_issues = db.query(models.Issue).filter(models.Issue.status.in_(["PENDING", "VERIFIED", "ASSIGNED", "IN_PROGRESS", "REOPENED"])).count()
    resolved_issues = db.query(models.Issue).filter(models.Issue.status.in_(["RESOLVED", "CLOSED"])).count()
    
    # Category Distribution
    category_counts = db.query(models.Issue.category, func.count(models.Issue.id)).group_by(models.Issue.category).all()
    distribution = [{"name": c[0] or "Uncategorized", "value": c[1]} for c in category_counts]
...
def get_leaderboard(type: str, db: Session = Depends(get_db)):
    ...
    leaderboard = []
    for u in users:
        impact_score = (u.total_reports * 5) + (u.resolved_reports * 10) + (u.total_verifications * 2)
...
def get_ai_insights(current_user: models.User = Depends(RoleChecker("Admin")), db: Session = Depends(get_db)):
    ...
    category_counts = db.query(models.Issue.category, func.count(models.Issue.id)).group_by(models.Issue.category).all()
    distribution = [{"name": c[0] or "Uncategorized", "count": c[1]} for c in category_counts]
